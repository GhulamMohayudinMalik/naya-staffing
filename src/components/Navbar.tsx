...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 z-50 w-full transition-all duration-500 ${scrolled
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-500 relative ${scrolled ? 'py-2 lg:py-4' : 'py-4 lg:py-6'}`}>
        <div className="flex items-center justify-between h-16 lg:h-20 gap-4">
          ...
          <div className="hidden lg:flex items-center justify-center space-x-3 xl:space-x-6 flex-grow px-2">
            ...
            {/* Employers Dropdown */}
            <div className="relative group/nav">
              <Link href="/employer-solutions" className="flex items-center font-semibold text-[10px] xl:text-[11px] uppercase tracking-wider py-8 text-white/90 hover:text-gold transition-colors whitespace-nowrap">
                Employers
                <ChevronDown className="ml-1.5 w-3 h-3 group-hover/nav:rotate-180 transition-transform duration-300" />
              </Link>
              <div className="absolute left-1/2 -translate-x-1/2 mt-0 w-64 rounded-2xl shadow-2xl bg-[#0B132B]/95 backdrop-blur-2xl border border-white/10 opacity-0 invisible group-hover/nav:opacity-100 group-hover/nav:visible transition-all duration-300 transform origin-top translate-y-4 group-hover/nav:translate-y-0 z-50 overflow-hidden">
                <div className="py-4 px-2">
                  ...
                  <Link href="/permanent-recruitment" className="block px-6 py-3 text-[10px] font-black tracking-widest text-gray-400 hover:bg-white/5 hover:text-gold rounded-xl transition-all uppercase">Direct Hire</Link>
            ...
            <div className="relative group/nav">
              <Link href="/job-openings" className="flex items-center font-semibold text-[10px] xl:text-[11px] uppercase tracking-wider py-8 text-white/90 hover:text-gold transition-colors whitespace-nowrap">
                Job Seekers
                <ChevronDown className="ml-1.5 w-3 h-3 group-hover/nav:rotate-180 transition-transform duration-300" />
              </Link>
              <div className="absolute left-1/2 -translate-x-1/2 mt-0 w-72 rounded-2xl shadow-2xl bg-[#0B132B]/95 backdrop-blur-2xl border border-white/10 opacity-0 invisible group-hover/nav:opacity-100 group-hover/nav:visible transition-all duration-300 transform origin-top translate-y-4 group-hover/nav:translate-y-0 z-50 overflow-hidden">
                <div className="py-4 px-2">
                  ...
                  <Link href="/register-upload-resume" className="block px-6 py-3 text-[10px] font-black tracking-widest text-gray-400 hover:bg-white/5 hover:text-gold rounded-xl transition-all uppercase">Upload Resume</Link>
                </div>
              </div>
            </div>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 z-40 bg-[#040814] transition-all duration-500 lg:hidden ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="space-y-2">
                  <Link href="/job-openings" onClick={() => setIsMenuOpen(false)} className="text-xl font-bold text-white/80 hover:text-gold block">Job Seekers</Link>
                  <Link href="/register-upload-resume" onClick={() => setIsMenuOpen(false)} className="text-sm font-semibold text-gold/80 hover:text-gold ml-4 block">→ Upload Resume</Link>
                </div>
