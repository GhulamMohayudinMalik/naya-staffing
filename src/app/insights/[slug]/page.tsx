...
export default function InsightReadingPage() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-navy-dark text-white flex items-center justify-center text-xl animate-pulse">Loading Intel...</div>;
  }
  ...
  if (error || !post) {
    return (
      <div className="min-h-screen bg-navy-dark text-white flex flex-col items-center justify-center space-y-6">
        ...
        <Link href="/insights" className="text-gold uppercase tracking-widest text-xs hover:text-white transition-colors">
  ...
  return (
    <div className="min-h-screen bg-navy-dark flex flex-col font-sans selection:bg-gold selection:text-navy-dark">
      <main className="max-w-4xl mx-auto py-32 px-4 sm:px-6 lg:px-8 w-full">
        <Link href="/insights" className="inline-flex items-center gap-3 text-gold hover:text-white transition-colors uppercase tracking-[0.3em] text-xs font-black mb-12">
          <ArrowLeft className="w-4 h-4" /> Back to Circuit
        </Link>
        ...
        <article className="space-y-12">
          ...
          <div className="w-full aspect-video rounded-3xl overflow-hidden glass-panel border-white/10 shadow-2xl relative">
            <img 
              src={post.image_url || "https://breezyrecruit.com/wp-content/uploads/2025/05/Become-a-client-scaled.webp"} 
              alt={post.title} 
